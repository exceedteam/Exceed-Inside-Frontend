...
class Events extends React.Component {
  ...
  componentDidUpdate() {
    const cameArr = this.props.events.filter(
      (event) => event.authorId === this.decoded
    );
    const currentArr = this.state.eventsOfUser;
    if (
      cameArr.length !== currentArr.length ||
      JSON.stringify(cameArr) !== JSON.stringify(currentArr)
    ) {
